...
class ChannelConfigurationViewController: BaseViewController, UITextFieldDelegate {
    ...
    func connectAsRole(userId: String = "b9db2535-9d1f-45af-9da9-f0978eeffc5f") {
        ...
        guard let channelNameValue = self.channelName?.trim(), !channelNameValue.isEmpty else {
            popUpError(title: "Missing Required Fields", message: "Channel name is required for WebRTC connection")
            return
        }
        guard let awsRegionValue = self.regionName?.trim(), !awsRegionValue.isEmpty else {
